...
class Save extends Code
{
    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            try {
                $countGiftCardUse = 0;
                if(!empty($this->_helperData->getCountGiftCardUse())){
                    $countGiftCardUse = 1;
                }
                if(empty($giftCard->getCountGiftCardUse()))
                {
                    $giftCard->setData('count_giftcard_use', $countGiftCardUse);
                }
    ...
    protected function prepareData(&$data)
    {
        ...
        if (isset($data['information']['mpgiftcard_config_is_can_refund'])) {
            $data['is_can_refund'] = $this->_helperData->getIsCanRefund();
            $data['is_config_refund']= 1;
        } else {
            $data['is_config_refund']= 0;
        }
