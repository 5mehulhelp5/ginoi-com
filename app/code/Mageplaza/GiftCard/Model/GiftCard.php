...
class GiftCard extends AbstractModel implements IdentityInterface, GiftCodeInterface
{
     ...
     * @return $this
     */
    public function addAmountUsed($amount)
    {
        $this->setAmountUsed($this->getAmountUsed() + $amount);
    ...
    public function spentForOrder($amount, $order, $quote)
    {
        ...
        $this->addAmountUsed($amount);

        $this->_helper->setValueMaxGiftCardUse($this);
    ...
    public function afterSave()
    {
        ...
        if ($this->getAction()) {
            ...
            } elseif ($this->getAction() == Action::ACTION_REFUND){
                $this->_historyFactory->create()->setGiftCard($this)->save();
            }
        }
     ...
     * @throws Exception
     */
    public function validateRefundBalance()
    {
        $giftCardDate = $this->_helper->getGiftCardTimeZone($this);
        $datetime     = new DateTime('', $giftCardDate);
        $datetime->format('Y-m-d');
        if(!$this->getIsCanRefund() && $this->getCountGiftCardUse() === 'used'){
            return false;
        } else if($this->getIsCanRefund() != 'used' && $this->getCountGiftCardUse()){
            $this->_helper->setValueMaxGiftCardUse($this);
            return true;
        }
        if($this->getExpiredAt() && $this->getIsCanRefund()){
            if($this->getExpiredAt() > $datetime->format('Y-m-d')) {
                $this->_helper->setValueMaxGiftCardUse($this);
                return true;
            }
        } else if(empty($this->getExpiredAt()) && $this->getIsCanRefund()){
            $this->_helper->setValueMaxGiftCardUse($this);
            return true;
        }
        return false;
     ...
     */
    public function getIsCanRefund()
    {
        return $this->getData(self::IS_CAN_REFUND);
    }
     ...
     * @return GiftCard
     */
    public function setIsCanRefund($value)
    {
        return $this->setData(self::IS_CAN_REFUND, $value);
    }
     ...
     * @return GiftCard
     */
    public function setCountGiftCardUse($value)
    {
        return $this->setData(self::COUNT_GIFTCARD_USE,$value);
    }

    /**
     * @return array|mixed|null
     */
    public function getCountGiftCardUse()
    {
        return $this->getData(self::COUNT_GIFTCARD_USE);
    }
}
