...
class Transaction extends AbstractModel implements IdentityInterface
{
    ...
    public function redeemGiftCard($customer, $giftCard)
    {
        if (!$giftCard->isActive()) {
            throw new LocalizedException(__(
                'Cannot redeem giftsetBalance card. Gift Card "%1" does not available.',
                $giftCard->getCode()
            ));
        }

        $giftCard->setAmountUsed($giftCard->getAmountUsed() + $giftCard->getInitBalance());
        $this->_dataHelper->setValueMaxGiftCardUse($giftCard);
    ...
    public function afterSave()
    {
        ...
        if ($emailHelper->isEmailEnable(Email::EMAIL_TYPE_CREDIT)) {
            ...
            $emails = explode(' ', $customer->getEmail()) + $emailHelper->getEmail();
