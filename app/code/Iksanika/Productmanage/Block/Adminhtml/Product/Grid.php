...
class Grid extends \Magento\Backend\Block\Widget\Grid\Extended
{
    ...
    public $_taxClassSourceProduct;
    public $_eavEntityOptCollection;

    public $_objectManager;
    public $_helper;
    public $_helperCategory;
    public $_productModel;
    public $_catalogSearch;
...
//                    'class' => 'action-secondary iks-btn iks-search',
    ...
    {
        return !$this->getParam('q', null) ? null : urldecode($this->getParam('q'));
    }
...
//                    'column_css_class' => 'col-attr-name'
        ...
        if($this->_helper->colIsVisible('category')) 
        {
            $this->addColumn('category',
                array(
                    ...
                    'type' => 'options',
                    'options' => $this->_helperCategory->getCategoriesOptionsForFilter(),
            ));
        }
        ...
        if ($this->moduleManager->isEnabled('Magento_CatalogInventory')) {
            if ($this->moduleManager->isEnabled('Magento_InventoryApi')) {
                if($this->_helper->colIsVisible('salable_qty')) {
                    $this->addColumn(
                        'salable_qty',
                        [
                            'header' => __('Salable Qty'),
                            'type' => 'number',
                            'index' => 'sku',
                            'filter' => false,
                            'renderer' => 'Iksanika\Productmanage\Block\Widget\Grid\Column\Renderer\Salable',
...
//                        'options' => $this->_websiteFactory->create()->getCollection()->toOptionArray(),
...
//        foreach(self::$columnSettings as $code => $true) 
        foreach($this->_helper->getColumnSettings() as $code => $true) 
        {
                ...
                $code != 'associated_groupped_ids' && $code != 'associated_configurable_ids')
            {
                ...
                {
                    $column['type']             =   'price';
                    // declined temporary as moved from renderer to plugin base override
...
//                    $column['renderer'] = '\Iksanika\Productmanage\Block\Widget\Grid\Column\Renderer\GroupPrice';
                ...
                'actions' => [
                    [
                        ...
                        'url' => [
                            ...
                            '_iksCalled' => ['iksApmBack' => '1']
                        ],
                        'field' => 'id'
                    ]
                ],
                'renderer' => 'Iksanika\Productmanage\Block\Widget\Grid\Column\Renderer\Action',
                'filter' => false,
                'sortable' => false,
                'index' => 'link_edit',
    ...
    */        
            if (($_auth->isAllowed('Iksanika_Productmanage::ma_category_add') ||
                ...
                ($this->_scopeConfig->getValue('iksanika_productmanage/massactions/enableaddcategory') ||
                 $this->_scopeConfig->getValue('iksanika_productmanage/massactions/enableremovecategory') ||
                 $this->_scopeConfig->getValue('iksanika_productmanage/massactions/enablereplacecategory')
                ))
            {
                ...
                $_auth->isAllowed('Iksanika_Productmanage::ma_up_sells_clear')))
            {
                ...
                if (($_auth->isAllowed('Iksanika_Productmanage::ma_related_to_each_other') ||
                    $_auth->isAllowed('Iksanika_Productmanage::ma_related_to_add') ||
                    $_auth->isAllowed('Iksanika_Productmanage::ma_related_to_clear')) &&
                    $this->_scopeConfig->getValue('iksanika_productmanage/productrelator/enablerelated')
                )
                {
                ...
                if (($_auth->isAllowed('Iksanika_Productmanage::ma_cross_sell_to_each_other') ||
                    $_auth->isAllowed('Iksanika_Productmanage::ma_cross_sell_to_add') ||
                    $_auth->isAllowed('Iksanika_Productmanage::ma_cross_sell_to_clear')) &&
                    $this->_scopeConfig->getValue('iksanika_productmanage/productrelator/enablecrosssell')
                )
                {
                ...
                if(($_auth->isAllowed('Iksanika_Productmanage::ma_up_sells_add') ||
                   $_auth->isAllowed('Iksanika_Productmanage::ma_up_sells_clear')) &&
                   $this->_scopeConfig->getValue('iksanika_productmanage/productrelator/enableupsell'))
                {
...
//        if($this->_scopeConfig->getValue('productmanage/massactions/categorynames', $this->_helper->getStoreId()))
        if($this->_scopeConfig->getValue('iksanika_productmanage/massactions/categorynames'))
        {
            ...
            */
            $element['category_value']['values']  =   $this->_helperCategory->getTree();
        }
...
//                        $colContent = implode(',', $item->getCategoryIds());
                    if(is_array($colContent) || $colIndex == 'category_ids')
                        $colContent = implode(',', $item->getCategoryIds());
                    $data[] = '"'.str_replace(array('"', '\\'), array('""', '\\\\'), !$colContent? '' : $colContent).'"';
                }
            }
            $csv.= implode(',', $data)."\n";
        }
