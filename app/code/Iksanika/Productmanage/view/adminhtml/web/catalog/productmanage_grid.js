...
//                    Element.addClassName(this.rows[row], 'even');
                    Element.addClassName(this.rows[row], '_odd-row');
                }
...
varienGridMassaction.prototype = {
    ...
    findCheckbox: function(evt) {
        if(['a', 'input', 'select', 'textarea', 'img'].indexOf(Event.element(evt).tagName.toLowerCase())!==-1) {
            return false;
        }
        checkbox = false;
...
//console.debug(item);
//console.debug(this.formFieldName);
...
//console.log('ITEM: '+item);
            if(multiFields != null) 
            {
                ...
                {
                    var title = jQuery('tr input[name=product][value='+item+']').parent().parent().parent().attr('title');
                    itemValue = jQuery("tr[title='"+title+"'] [name="+multiFields[i]+"]").val();

//                    jQuery('tr input[name=product][value=5]').parent().parent().parent().attr('title');
                    
//                    itemValue = $('tr input[name=product][value='+item+'] [name='+multiFields[i]+']').val();

                    fieldsHtml += "<textarea name=\""+multiFields[i]+"[]\" style=\"display:none;\">"+itemValue+"</textarea>";
                }
            } else {
//                fieldsHtml += this.fieldTemplate.evaluate({
                fieldsHtml += this.fieldTemplate({
                    name: fieldName+'[]', 
                    value: item
                });
            }
