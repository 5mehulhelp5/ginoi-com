...
//                $verifiedCategories = array_flip($category->verifyIds($categoryIdsAdd));
                $verifiedCategories = $category->verifyIds($categoryIdsAdd);
                ...
                foreach($productIds as $productId)
                {
                    ...
                    if($product)
                    {
                        ...
                        $productIdsAdded[] = $productId;
                    }

                }

                $this->messageManager->addSuccessMessage(__('A total of #%1 product(s) (prod ids: %2) has been added to category id: %3.', (count($productIds)), implode(', ', $productIdsAdded), implode(', ', $verifiedCategories)));
