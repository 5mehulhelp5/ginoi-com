...
class MassRelatedTo extends \Magento\Catalog\Controller\Adminhtml\Product
{
    public $_helperData;
        ...
        if(is_array($productIds))
        {
            ...
            foreach($productIds as $itemId => $productId)
            {
                try {
                    ...
                    $product->setProductLinks($this->_helperData->addProductsLinksByType($product, $productIds2, 'related'));
                    $product->save();
                    $isUpdated ++;
    ...
    {
        return $this->_authorization->isAllowed('Iksanika_Productmanage::ma_related_to_add');
    }
}
