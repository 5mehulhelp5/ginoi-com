...
class MassRelatedEachOther extends \Magento\Catalog\Controller\Adminhtml\Product
{
        ...
        if(is_array($productIds))
        {
            ...
            foreach($productIds as $itemId => $productId)
            {
                try {
                    ...
                    $product->setProductLinks($this->_helperData->addProductsLinksByType($product, $productIds, 'related'));
                    $product->save();
                    $isUpdated ++;
            ...
            $this->messageManager->addSuccessMessage(__('Total of %1 out of %2 record(s) were successfully related to each other.', $isUpdated, count($productIds)));
        }else
        {
    ...
    {
        return $this->_authorization->isAllowed('Iksanika_Productmanage::ma_related_to_each_other');
    }
}
