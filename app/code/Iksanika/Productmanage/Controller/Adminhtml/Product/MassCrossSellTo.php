...
class MassCrossSellTo extends \Magento\Catalog\Controller\Adminhtml\Product
{
        ...
        if(is_array($productIds))
        {
            ...
            foreach($productIds as $itemId => $productId)
            {
                try {
                    ...
                    $product->setProductLinks($this->_helperData->addProductsLinksByType($product, $productIds2, 'crosssell'));
                    $product->save();
                    $isUpdated ++;
            ...
            $this->messageManager->addSuccessMessage(__('Total of %1 out of %2 record(s) were successfully crosssell to specified list of products.', $isUpdated, count($productIds)));
        }else
        {
    ...
    {
        return $this->_authorization->isAllowed('Iksanika_Productmanage::ma_cross_sell_to_add');
    }
}
