...
class MassUpSellTo extends \Magento\Catalog\Controller\Adminhtml\Product
{
        ...
        if(is_array($productIds))
        {
            ...
            foreach($productIds as $itemId => $productId)
            {
                try {
                    ...
                    $product->setProductLinks($this->_helperData->addProductsLinksByType($product, $productIds2, 'upsell'));
                    $product->save();
                    $isUpdated++;
            ...
            $this->messageManager->addSuccessMessage(__('Total of %1 record(s) out of %2 were successfully linked as upsell.', $isUpdated, count($productIds)));
        }else
        {
    ...
    {
        return $this->_authorization->isAllowed('Iksanika_Productmanage::ma_up_sells_add');
    }
}
