...
class UpdateSpecialPrice extends \Magento\Catalog\Controller\Adminhtml\Product
{
    /**
        ...
        foreach ($productIds as $itemId => $productId)
        {
            try {
                ...
                if ($productId)
                {
                    ...
                    $product->setData('special_price', $this->_helperData->recalculatePrice($product->getData('special_price'), $columnValuesForUpdate))->save();
...
//            $this->_productPriceIndexerProcessor->reindexList($productIds);
    ...
    {
        return $this->_authorization->isAllowed('Iksanika_Productmanage::ma_update_special_price');
    }
}
